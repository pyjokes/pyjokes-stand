...
function run-flake8-analysis {
    echo "Running flake8 analysis ..." && ( flake8 ${LIB_FILES} )
}
...
function run-pylint-analysis {
    echo "Running pylint analysis ..." && ( pylint $(find ${PROJECT_FILES} -iname ${LIB_FILES}) )
}
...
function run-pydocstyle-analysis {
    echo "Running pydocstyle analysis ..." && ( pydocstyle --explain --count ${PY_FILES} ${LIB_FILES}} )
}
...
function start-analysis {
    ...
    run-pydocstyle-analysis || store-failures "pydocstyle analysis is failed!"
}


function run-code-assessment {
